        ...
        result.forEach(item => {
            ...
            card.innerHTML = `
                <div class="product-card-image" aria-hidden="true">
                    <img class="product-image product-image-primary" src="${escapeHtml(primaryImage)}" alt="${escapeHtml(getLocalizedProduct(item).name)}" loading="lazy" decoding="async">
                    <img class="product-image product-image-hover" src="${escapeHtml(hoverImage)}" alt="" loading="lazy" decoding="async">
                </div>
                <h3 class="product-title">${escapeHtml(getLocalizedProduct(item).name)}</h3>
                ${item.model ? `<div class="product-model">Model: ${escapeHtml(item.model)}</div>` : ""}
                <p class="product-description">${escapeHtml(getLocalizedProduct(item).description)}</p>
